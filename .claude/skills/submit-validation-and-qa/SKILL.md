...
verification commands per change type, and the honest gaps (dated 2026-07-08).
...
|---|---|---|---|
| `npm test` | Full suite: 16 tests (11 pure engine + 5 live isolation) | 16 passed | Isolation half needs a live `DATABASE_URL` Postgres with base **and dev** seeds (`bahama-brokerage` + `island-customs` orgs). Fails fast with "Dev seed missing" otherwise. |
| `npx vitest run tests/calculations.test.ts` | Calculation engine only — pure, deterministic | 11 passed | **No DB needed.** Safe anywhere. |
...
| `npx tsx scripts/smoke.ts` | E2E: calculate → duty math spot checks → mock BEAIP submit → status transition | Exits 0, prints "Smoke passed" | Live seeded DB. **MUTATES data** — see warning below. Not safe against a database whose data you care about. |
...
| `npx tsx .claude/skills/submit-calculations-and-money/scripts/verify-example.ts` | The sibling skill's worked example matches the real engine (18 asserted values) | Exit 0 | No DB. Run from repo root. |
...
2. Calculates with `apportionmentBasis: 'VALUE'` and hard-asserts:
   - line 1 (rum) `dutyAmount === '6000.00'` (600 L × $10/L SPECIFIC duty)
   - `totalCifValue − totalFobValue === '2315.00'` (freight 1850 + insurance 320 + other 145, apportioned to the penny)
3. Submits declaration type `C13` via **mock** BEAIP; asserts shipment status
   becomes `SUBMITTED`.
4. Resets the shipment back to `DRAFT` (`submittedAt: null`) for demo reuse.
...
3. `npx tsx scripts/smoke.ts` (seeded DB) — the 6000.00 / 2315.00 spot checks
   must still hold.
4. `npx tsx .claude/skills/submit-calculations-and-money/scripts/verify-example.ts`
...
### Touched BEAIP (`src/lib/beaip/**`, `declarations.service.ts`)
1. `npx tsx scripts/smoke.ts` — mock-mode happy path.
2. Exercise the **REJECT path**: the mock client deterministically rejects any
   declaration whose `brokerReference` contains `REJECT`
   (`src/lib/beaip/mock-client.ts`) — verify the entry lands as `REJECTED`
   and the shipment does not advance.
3. Know the limit: this only certifies the **mock**. The production client
   (`production-client.ts`) has never been run against the real service (§5).
...
## 5. Gaps — honest, as of 2026-07-08
...
| **Repo is not under version control** (no `.git`) | No history, no bisect, no cheap revert. A bad edit is only recoverable from backups. Be extra conservative with destructive changes. |
| **No HTTP/route-layer tests** | Status codes, zod validation errors, auth middleware, and route wiring are untested. |
| **Production BEAIP client unverified** | `src/lib/beaip/production-client.ts` has never been exercised against the real BEAIP service; only the mock is tested. First real submission is uncharted. |
...
| **Services with zero direct test coverage:** `auth.service.ts`, `billing.service.ts`, `catalog.service.ts`, `hs-codes.service.ts`, `invoices.service.ts`, `shipments.service.ts` | Only the pure engine has unit tests; `calculations.service.ts` and `declarations.service.ts` are exercised indirectly (and only on the happy path) by `scripts/smoke.ts`. Everything else: typecheck only. |
| **Smoke covers one happy path** | One org, one shipment, mock submit, no REJECT-path automation, no concurrency. |

## 6. Golden inventory (certified numbers and fixtures)

**Smoke-script certified numbers** (seeded shipment `SHP-2026-00001`, org
`bahama-brokerage`, 3 mixed-basis lines: rum SPECIFIC, t-shirts AD_VALOREM,
fridge with levy):
- Rum line duty: **6000.00** (600 L × $10/L, value-independent)
- Total CIF − total FOB: **2315.00** exactly (1850.00 freight + 320.00 insurance + 145.00 other)
- Post-submit status: `SUBMITTED`; script leaves it `DRAFT`.

**The 11 calc tests certify** (`tests/calculations.test.ts`):
...
colliding numbers); **43-code HS subset** in `prisma/seed.ts` (`HS_SUBSET`);
mock BEAIP force-reject hook: `brokerReference` containing `REJECT`.

## Provenance and maintenance

- Every command, count, and number above was read from the repo on 2026-07-08: `tests/calculations.test.ts`, `tests/tenant-isolation.test.ts`, `tests/setup.ts`, `vitest.config.ts`, `scripts/smoke.ts`, `package.json`, `prisma/seed.ts`, `prisma/seed.dev.ts`, `src/lib/beaip/mock-client.ts`, README "Verifying everything".
