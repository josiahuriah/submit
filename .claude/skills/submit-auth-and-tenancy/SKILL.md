...
**The domain-critical rule: `shipments:submit` requires BROKER+.** Clerks
prepare shipments; only licensed brokers file declarations with customs.
Enforced at `src/app/api/shipments/[id]/submit/route.ts` via
`{ permission: 'shipments:submit' }`. If someone asks "why can a clerk not
submit" — this is by design, do not relax it.
