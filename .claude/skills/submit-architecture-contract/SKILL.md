...
Submit (v0.3.0, repo root `/Users/joshduncanson/Downloads/submit`) is a
multi-tenant SaaS for Bahamian customs brokerages: prepare shipments,
calculate duty/VAT/levy/excise to the cent, file declarations through BEAIP
(the Bahamas electronic single-window customs platform, operated by
CrimsonLogic), and bill clients. Stack: Next.js 15 App Router, React 19,
TypeScript, Prisma 7 (driver adapter), PostgreSQL (Neon), Zod v3,
decimal.js, Vitest, tsx. README.md is the document of record — `docs/`
exists but is empty.
...
| 6 | Permission checks happen in `withAuth(handler, { permission })` at the route boundary, mapped via `src/lib/auth/rbac.ts` (Permission string → minimum role, `VIEWER < CLERK < BROKER < ADMIN < OWNER`). Domain-critical rule: `shipments:submit` requires BROKER+ | Clerks prepare, licensed brokers file — a legal/regulatory requirement, not a convenience | Stated — README "RBAC" + comment in `submit/route.ts`; derived — checks live in routes, NOT in services |
| 7 | Calculation engine (`src/lib/calculations/`) is pure, deterministic, decimal.js-only. Largest-remainder apportionment by VALUE or WEIGHT; duty bases AD_VALOREM / SPECIFIC / COMPOUND (greater-of); VAT on (CIF + duty + excise + levy); processing fee 1% of CIF clamped $10–$750 plus VAT on the fee; ROUND_HALF_UP to 2dp | Floats never touch money (`money.ts` calls native floats "banned from this codebase for money"); per-line cents must sum exactly to the shipment charge | Stated — README + `money.ts` comments |
| 8 | Rate freezing: applied rates are copied onto each LineItem at calculation time | A shipment's numbers must remain auditable after tariff changes | Stated — README |
| 9 | Staleness guard: any invoice/line-item mutation clears `shipment.calculatedAt` (`invoices.service.ts:46`); `declarations.service.ts` refuses submission unless `calculatedAt` exists and `calculatedAt >= updatedAt` | You may never file numbers the broker has not seen recomputed | Stated — README + `invoices.service.ts` / `declarations.service.ts` comments |
| 10 | BEAIP behind one `BeaipClient` interface; mock vs production SOAP implementation chosen by `BEAIP_MODE` via `getBeaipClient()` factory. Every request/response payload persisted verbatim on `CustomsEntry.requestPayload/responsePayload`, rejections included | "Flipping BEAIP_MODE=production (with credentials) is the entire cutover"; "Filed declarations are legal documents; the audit trail is non-negotiable" | Stated — README + `src/lib/beaip/index.ts` comment. NOTE: the production client is UNVERIFIED against the real service (awaits CrimsonLogic WSDL docs) |
...
| Staleness guard intact | `grep -rn "calculatedAt" src/server/services/invoices.service.ts src/server/services/declarations.service.ts` | `invoices.service.ts` nulls `calculatedAt` on every invoice/line-item mutation; `declarations.service.ts` rejects when `!calculatedAt` or `calculatedAt < updatedAt` |
| Submit requires BROKER+ | `grep -rn "shipments:submit" src/lib/auth/rbac.ts src/app/api` | rbac maps it to `'BROKER'`; the submit route passes `{ permission: 'shipments:submit' }` |
...
| BEAIP payloads persisted verbatim | `grep -n "requestPayload\|responsePayload" src/server/services/declarations.service.ts` | Written on submit AND on refresh |
...
| Tests still pass | `npm test` | 16 tests: 11 in `tests/calculations.test.ts`, 5 in `tests/tenant-isolation.test.ts` (the latter needs a live DB). Then `npx tsx scripts/smoke.ts` for end-to-end |
...
**Route handlers** (`src/app/api/`, 26 routes) may ONLY: wrap themselves in
...
state of its own: calculations, beaip clients, errors, env, api-response,
auth plumbing, and the tenant client factory itself.

## Data flow (happy path: calculate → submit)
...
4. `POST /api/shipments/:id/submit` (BROKER+) → `declarations.service`
   checks the staleness guard, builds the declaration, calls
   `getBeaipClient().submit(...)` (mock or production per `BEAIP_MODE`),
   persists request/response payloads verbatim on CustomsEntry, writes
   audit, updates shipment status. The mock returns `BS-YYYY-E######`
   references; a `brokerReference` containing `REJECT` forces the rejection
   path for testing.
5. `POST /api/customs-entries/:id/refresh` polls status; the response
   payload is again persisted.
...
3. **Production BEAIP client is UNVERIFIED.** `production-client.ts` is a
   complete SOAP client (fast-xml-parser, WS-Security UsernameToken, 60s
   timeout) but has never been exercised against the real service — README
   says the namespace/action/element names in
   `buildEnvelope()`/`parseResponse()` await CrimsonLogic's WSDL
   documentation. Do not describe BEAIP integration as done.
4. **UI is demo-only.** `src/app/` contains just login + dashboard (README:
   "demo UI"). The product surface is the 26-route API.
5. **HS code data is a 43-code subset.** `prisma/seed.ts` loads a
   representative subset covering every duty basis; the full 1,544-code
   2023 Tariff Schedule extraction has not been dropped at
   `prisma/data/hs-codes.json` (`prisma/data/` is empty as of 2026-07-08).
...
- Route count (expect 26): `find src/app/api -name route.ts | wc -l`
...
- Still not a git repo: `ls -a | grep '^\.git$'`
- Full HS dataset still missing: `ls prisma/data/`
- Test counts: `npm test` (expect 16).
