...
description: >
  API route-handler contract for the Submit codebase (26 implemented routes under
...
Date-stamped 2026-07-08. Everything below is mined from the 26 implemented `route.ts`
...
## Route inventory (26 route files, 35 exported handlers)
...
| POST | /api/shipments/:id/status | shipments:write | status machine in service |
| POST | /api/shipments/:id/calculate | shipments:write | |
| POST | /api/shipments/:id/submit | **shipments:submit** | BROKER+ only; body optional (`.catch(() => ({}))`) |
...
| POST | /api/customs-entries/:id/refresh | **shipments:read** | mutating POST behind a read permission — see Findings |
...
4. **Respond helpers only** — `ok(data, { meta?, status? })` (200), `created(data)` (201).
   Never hand-roll `NextResponse.json` in a route.
...
- Optional body: `await req.json().catch(() => ({}))` then parse a schema whose fields
  all have defaults (`src/app/api/shipments/[id]/submit/route.ts`).
...
| invoices/line items | `invoiceCreateSchema` / `invoiceUpdateSchema` (`.partial().omit({ shipmentId })`), `lineItemCreateSchema` / `lineItemUpdateSchema` (`.partial().omit({ invoiceId })`) | currency locked to `z.literal('BSD')`; unitPrice allows 4 dp, totalValue is `money` (2 dp) |
| calculation/submission | `calculateOptionsSchema` (`apportionmentBasis: VALUE\|WEIGHT`), `submitDeclarationSchema` (`declarationType: C13\|C14\|C17\|C18\|OTHER`, default C13) | all-default fields → routes tolerate empty body |
...
### Known gap: HS-code format asymmetry (verified 2026-07-08)

`lineItemCreateSchema.hsCode` is only `z.string().min(4).max(15)` (schemas.ts:136), but the
mock BEAIP client rejects any line whose code fails `/^\d{4}\.\d{2}(\.\d{2})?$/`
(`src/lib/beaip/mock-client.ts:23`). Consequence: a malformed code like `"abcd"` or
`"22084010"` (no dots) is accepted at line-item creation and only fails at
`POST /shipments/:id/submit`, surfacing as a submission-time rejection instead of a 400
at entry time. If tightening, align the Zod regex with the BEAIP contract (owner: Josiah).

## Layering rules (verified by grep, 2026-07-08)

- **Routes are thin.** Zero occurrences of `prisma`, `getPrisma`, or `createTenantClient`
  in any of the 26 route files — routes only receive the tenant-scoped `db` from `withAuth`
...
- **Services** (`src/server/services/*.ts`: auth, billing, calculations, catalog,
  declarations, hs-codes, invoices, shipments) own business rules (status machines,
...
6. **README**: update the "API surface (26 routes)" block and its count.

## Findings (audit of all 26 routes, 2026-07-08 — resolution owner: Josiah)

1. **`POST /api/customs-entries/:id/refresh` requires only `shipments:read`** yet it is a
   mutating POST (refreshes entry status, writes audit). Every other mutation requires a
   `:write`/`:submit`/`billing:write` permission. Likely intent: polling BEAIP status is
   "read-like", but it breaks the method↔permission convention. Decide and document.
2. **Suppliers reuse `clients:read`/`clients:write`** — there is no `suppliers:*` permission
   in rbac.ts. Acceptable (same VIEWER/CLERK levels) but non-obvious; a reviewer expecting
   `suppliers:write` will not find it.
3. **HS-code validation asymmetry** (detailed above): API accepts formats BEAIP will reject;
   failure is deferred to submission time.
4. **`GET /api/auth/me` passes no permission** — intentional (any authenticated role may
   introspect itself), but it is the only `withAuth` route without one; note it so audits
   don't flag it repeatedly.
5. **`GET /api/hs-codes/search` ignores the tenant `db`** — `hsCodesService.search(q, limit)`
...
6. **`GET /api/billing/invoices/:id` has no PATCH/DELETE/void sibling** — invoices can be
   created, sent, and paid but not voided via API despite a `VOID` status in the list filter
   enum. Gap, not a bug.

No envelope inconsistencies found: all 26 files use `ok`/`created`/`fail` exclusively.

## Provenance and maintenance

- Route count: `find src/app/api -name route.ts | wc -l` (expect 26).
