...
Updated 2026-08-08. Everything below is derived from the canonical repo. If code and this doc disagree, the code wins — then fix this doc.

The money engine includes `money.ts`, `apportionment.ts`, `measurement.ts`, and `duty-calculator.ts`; its executable spec is `tests/calculations.test.ts` (16 tests), with `calculations.service.ts` as the DB bridge.
...
| 6 | **Assessment outputs are BSD.** Invoice-currency FOB is multiplied by the invoice's BSD exchange rate before apportionment and tax. | `duty-calculator.ts` FOB map; `Invoice.exchangeRate` |
| 7 | **Rates and assessment facts are frozen onto LineItem**: duty/excise bases and rates, units, converted assessment quantities, BSD FOB, VAT and levy. Historical results are never recomputed after submission. | `calculations.service.ts`, schema LineItem rate block |
| 8 | **Staleness guard**: invoice/line mutations null `calculatedAt`; review artifact generation refuses missing/stale calculations. | `invoices.service.ts`, `declaration-artifacts.service.ts` |
| 9 | **Only DRAFT shipments can be recalculated or produce review artifacts.** | calculation/artifact services |
...
2. **Per line**: `CIF = toMoney(FOB + freightShare + insuranceShare + otherShare)`; then
   - `duty` per basis: `AD_VALOREM` = CIF × rate; `SPECIFIC` = assessment quantity × rate; `COMPOUND` = greater of those; `ADDITIVE` = their sum.
   - `excise` is a separate charge with `NONE` / the same four bases. It never borrows the duty basis. `levy = CIF × levyRate`.
...
3. **SPECIFIC charges ignore CIF entirely** and use an explicit assessment quantity. `measurement.ts` converts package/bottle volumes using the supplied Customs worksheet factors (`litres × 0.22`, US fl oz ÷ 153.6, imperial fl oz ÷ 160) and converts strength to proof gallons. Missing required measurement data is a 422 business-rule refusal, never a fallback to commercial quantity.
...
2. Resolve each distinct HS code's rate **as of `Shipment.declarationDate`** (`effectiveFrom <= date` and `effectiveTo IS NULL OR effectiveTo > date`), newest first. Excisable chapters 22/24/27/87 require `isVerified=true`.
3. Resolve duty and excise assessment quantities, then map DB rows → engine inputs, stringifying every Decimal and applying invoice exchange rate.
...
Two mechanisms, enforced before review artifact generation:
...
2. **Timestamp comparison** — shipment-level edits bump `updatedAt`. Artifact generation refuses when `calculatedAt < updatedAt` by more than 5,000 ms. The declaration-profile action reprices an already-calculated draft after saving.

Artifact errors say to run/re-run the calculation. The entry page disables review XML while totals are absent.
...
Run the executable spec (16 tests):
...
End-to-end (needs DB + seed data; exercises calculation → review artifact,
spot-checks current rum excise = 1716.00 and exact CIF−FOB = 2315.00 without
calling a Customs endpoint):
...
```bash
npx tsx .Codex/skills/submit-calculations-and-money/scripts/verify-example.ts
```
...
- Sources: calculation modules including `measurement.ts`, tests, calculation/invoice/artifact services, schema, seed and the supplied Customs alcohol deck; re-verified 2026-08-08.
