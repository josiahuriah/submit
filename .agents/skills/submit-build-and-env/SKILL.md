...
description: >
  ...
  smoke test or `npm run dev` won't start.
---
...
- Future Customs endpoint onboarding → `submit-beaip-integration-campaign`
...
office `NAS`, and HS codes `2208.40.10`/`6109.10.00`/`8418.10.00` — all
...
`prisma/data/hs-codes.json` bundles the 1,544-code duty extraction. The global
seed overlays curated, effective-dated alcohol rate histories.
...
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` / `GOOGLE_REDIRECT_URI` | no | `''` | Required together for Google OAuth; password auth is unaffected. |
...
- `broker@bahamabrokerage.test` (BROKER)
...
press **Calculate**, then **Generate review XML**. No endpoint is called and
the shipment remains DRAFT.
...
- **smoke.ts** success looks like: resets SHP-2026-00001 to DRAFT and deletes
  its CustomsEntry rows, calculates (prints totals table), asserts rum excise
  `1716.00` (600 L × 0.22 × $13) and CIF−FOB apportionment exactly
  `2315.00`, generates a review XML artifact, and asserts status remains DRAFT.
...
7. **There is no Customs endpoint environment contract yet.** Do not add
   ...
   step-4 integration documentation and the campaign gates are revisited.
...
Updated 2026-08-08. Re-verify each section by reading, not trusting:
