...
Updated: 2026-08-08. Ground truth: `prisma/schema.prisma` (v2.2.0), all folders
under `prisma/migrations/`, `prisma/sql/rls.sql`,
...
|---|---|---|
| Organization | The tenant brokerage; carries distinct TIN, broker licence and TFP Company Registration Number. | parent of everything tenant-scoped |
...
| HSCodeRate | Effective-dated duty and excise bases/rates/units, VAT/levy, source URL/page/gazette and verification flag | → HSCode |
...
| Shipment | Core entity with declaration date/function/regime, transport/location/package fields, charges, rolled-up totals and `calculatedAt` | Client, Manifest?, CustomsOffice, User(createdBy) → Invoices, Documents, CustomsEntries |
...
- **Staleness guard**: `Shipment.calculatedAt` is cleared by any
  invoice/line-item mutation; review-artifact generation refuses until recalculated. New
  mutations touching invoice/line data must preserve this behavior.
- **Customs artifact audit trail**: `CustomsEntry.requestPayload` stores the exact
  exported XML string. `responsePayload` remains reserved for the future verified
  endpoint response. Treat each generated artifact as immutable; do not repurpose it.
