...
| Tenant model list consistent across layer 1 and layer 3 | inspect `TENANT_MODELS` in `src/lib/db/tenant-client.ts` and `tenant_tables` in `prisma/sql/rls.sql` | Same 15 models in both, including CustomsSubmissionBatch, CustomsSubmissionAttempt, and CustomsSubmissionStatusCheck. Adding an `organizationId`-bearing model to schema.prisma without adding it to BOTH lists silently un-isolates it |
| RLS is FORCE'd | `grep -c "FORCE ROW LEVEL SECURITY" prisma/sql/rls.sql` | ≥1 (applied per-table in a loop over the 15 tenant tables) |
...
- Targeted re-verification 2026-09-21: 26 models, 19 enums, and 15 tenant-scoped models after durable submission-status history was added.
